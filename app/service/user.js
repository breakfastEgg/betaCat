...
class UserService extends Service {
  ...
  async signup(user) {
    ...
    // const role = await service.role.show(user.role)
    // if (!role) {
    //   ctx.throw(404, 'role is not found')
    // }
    user.password = ctx.helper.Hash(user.password,'md5');
  ...
  async findByName(userName) {
    return this.ctx.model.User.findByName(userName);
  }
  ...
  async login(user) {
    const { ctx, service } = this
    const user = await this.findByName(user.userName);
    if(!user){
      ctx.throw(404, 'user not found')
    }
